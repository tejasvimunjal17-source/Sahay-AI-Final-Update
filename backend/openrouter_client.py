...
def chat_completion_with_tools(
    ...
    *,
    ...
    """PHASE 4 IMPLEMENTATION (Agentic AI upgrade — Companion tool-calling).

    Tool-calling variant of chat_completion(). Sends `messages` plus an
    OpenAI-style `tools` list and `tool_choice: "auto"`. Returns
    {"content": str | None, "tool_calls": list[dict]} instead of a bare
    string, since a tool-calling turn may return EITHER a final text
    reply OR a request to call one or more tools — both fields are
    always present so callers don't need an isinstance check.

    chat_completion() (no tools) is completely UNCHANGED by this
    function's existence — different function, different return type,
    sharing only the internal _post_with_retries()/_headers() helpers
    and the same status-code error handling.

    Raises the exact same OpenRouterNotConfiguredError /
    OpenRouterTimeoutError / OpenRouterRateLimitError /
    OpenRouterResponseError / OpenRouterError hierarchy as
    chat_completion() — never a raw requests exception.

    NOT LIVE-TESTED in this environment (no network access) — static/
    mock verification only, consistent with every prior phase.
    """
    if not OPENROUTER_CONFIG.is_configured:
        raise OpenRouterNotConfiguredError(
            "OPENROUTER_API_KEY, OPENROUTER_BASE_URL, and OPENROUTER_MODEL must all be "
    ...
    content = _strip_reasoning_blocks(raw_content) if isinstance(raw_content, str) and raw_content.strip() else None
